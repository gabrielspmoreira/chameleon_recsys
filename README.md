...
The initial version of CHAMELEON source code allows reproducibility of the experiments reported in the following paper (https://arxiv.org/abs/1808.00076) at the [DLRS'18](http://dlrs-workshop.org/), co-located with ACM RecSys.   
...
The CHAMELEON is a meta-architecture, in the sense of a reference architecture that collects together decisions relating
to an architecture strategy. It might be instantiated as different architectures with similar characteristics that fulfill a common task, in this case, news recommendations.
...
In this instantiation of the *Textual Features Representation (TFR)* sub-module from ACR module, 1D CNNs over pre-trained Word2Vec embeddings was used to extract features from textual items. 
...
After training, the *Article Content Embeddings* for news articles (NumPy matrix) is persisted into a Pickle dump file, for further usage by *NAR* module.

### Pre-processing data for the ACR module
For an example of the command for pre-processing articles text and metadata for the *ACR* module, see [acr_module/scripts/run_acr_preprocessing_gcom.sh](https://github.com/gabrielspmoreira/chameleon_recsys/blob/master/acr_module/scripts/run_acr_preprocessing_gcom.sh).

It allows to specify the path of a CSV containing articles text and metadata (*input_articles_csv_path*), the path of pre-trained word embeddings (*input_word_embeddings_path*) and exports articles data in TFRecords format to *output_tf_records_path*, including the dictionaries that mapped tokenized texts to ints (*output_word_vocab_embeddings_path*) and metadata categorical features label encoders (*output_label_encoders*).
...
### Training ACR module
The ACR module can be trained either locally ([run_acr_training_gcom_local.sh](https://github.com/gabrielspmoreira/chameleon_recsys/blob/master/acr_module/scripts/run_acr_training_gcom_local.sh)) (example below) or using GCP ML Engine ([run_acr_training_gcom_mlengine.sh](https://github.com/gabrielspmoreira/chameleon_recsys/blob/master/acr_module/scripts/run_acr_training_gcom_mlengine.sh)).

The path of pre-processed TFRecords is informed in *train_set_path_regex* parameter, as well as the other *ACR* exported assets (*input_word_vocab_embeddings_path* and *input_label_encoders_path*). The trained *Article Content Embeddings* (NumPy matrix), with the dimensions specified by *acr_embeddings_size*, are exported after training to *output_acr_metadata_embeddings_path*, for further usage by the *NAR* module.
...
For this reason, instead of using a softmax cross-entropy loss, the NAR module is trained to maximize the similarity between the *Predicted Next-Article Embedding* and the *User-Personalized Contextual Article Embedding* corresponding to the next article actually read by the user in his session (positive sample), whilst minimizing its similarity with negative samples (articles not read by the user during the session). With this strategy, a newly published article might be immediately recommended, as soon as its *Article Content Embedding* is trained and added to a repository.

### Pre-processing data for the NAR module
The NAR module expects TFRecords containing [SequenceExample](https://www.tensorflow.org/api_docs/python/tf/train/SequenceExample) of user sessions's context and clicked articles.

The following example command ([run_nar_preprocessing_gcom_dlrs.sh](https://github.com/gabrielspmoreira/chameleon_recsys/blob/master/nar_module/scripts/run_nar_preprocessing_gcom_dlrs.sh)) takes the path of CSV files with users sessions split by hour (*input_clicks_csv_path_regex*) and outputs the corresponding TFRecords.
...
The *NAR* module is trained and evaluated according to the following protocol, described in the DLRS'18 paper:
...
The following baseline methods (described in more detail in DLRS'18 paper) are also trained and evaluated in parallel, as benchmarks for CHAMELEON accuracy:
- **Co-occurrent**
- **Sequential Rules (SR)**
- **Item-kNN**
...
The *disable_eval_benchmarks* parameter disables training and evaluation of benchmark methods (to speed up). And the *save_eval_sessions_negative_samples* parameter saves a JSON lines file with the negative samples randomly sampled for each user session for a consistent evaluation of the *GRU4Rec* benchmark (see next sections).
...
As the example in ([run_nar_train_gcom_dlrs_local.sh](https://github.com/gabrielspmoreira/chameleon_recsys/blob/master/nar_module/scripts/run_nar_train_gcom_dlrs_local.sh)) 

The script takes as input the path
```bash
cd nar_module && \
DATA_DIR="[REPLACE WITH THE PATH TO UNZIPPED GCOM DATASET FOLDER]" && \
JOB_PREFIX=gcom && \
...
- *use_local_cache_model_dir* - TensorFlow logs and summaries are usually large and saving them directly to GCS adds a considerally slows down the training/evaluation process. When this parameter is present, TF model logs to a local folder in the worker machine.
...
As the example in ([run_nar_train_gcom_dlrs_mlengine.sh](https://github.com/gabrielspmoreira/chameleon_recsys/blob/master/nar_module/scripts/run_nar_train_gcom_dlrs_mlengine.sh)).
...
We managed to adapt its evaluation process to use the same protocol, metrics and negative samples randomly sampled during NAR training.

As the example in ([run_gru4rec_gcom_dlrs.sh](https://github.com/gabrielspmoreira/chameleon_recsys/blob/master/nar_module/scripts/run_gru4rec_gcom_dlrs.sh)).
